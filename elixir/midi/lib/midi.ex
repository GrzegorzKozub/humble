defmodule Midi do
  ...
  def from_file(name), do: %Midi{content: File.read!(name)}
end

defimpl Enumerable, for: Midi do
  # no smart way to implement member? or slice for a MIDI file
  # fall back to Enumerable default implementation
  def member?(%Midi{}, %Midi.Frame{}), do: {:error, __MODULE__}
  def slice(%Midi{}), do: {:error, __MODULE__}
